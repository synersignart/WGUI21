...
namespace MultiWiiGUIControls
{
    class MWGUIMotors : MWGUIControl
    {
        ...
        enum CopterType { Tri = 1, QuadP, QuadX, BI, Gimbal, Y6, Hex6, FlyWing, Y4, Hex6X, Octo8Coax, Octo8P, Octo8X, Airplane, Heli120, Heli90, Vtail, Hex6H, Singlecopter, Dualcopter };
        ...
        static int[,] coord_hex6h = { { 118, 191, 70 }, { 118, 69, 70 }, { 42, 191, 70 }, { 42, 69, 70 }, { 118, 131, 70 }, { 42, 131, 70 } };
        static int[,] coord_singlecopter = { { 85, 105, 70 }, { 120, 95, 70 }, { 75, 155, 70 }, { 50, 95, 70 }, { 10, 150, 100 } };
        static int[,] coord_dualcopter = { { 77, 88, 70 }, { 87, 97, 70 }, { 10, 120, 70 }, { 90, 120, 70 } };
        ...
        Bitmap bmpHex6H = new Bitmap(MultiWiiWinGUI.MWGUIControls.MWGUIControlsResources.hex6h);
        Bitmap bmpSinglecopter = new Bitmap(MultiWiiWinGUI.MWGUIControls.MWGUIControlsResources.singlecopter);
        Bitmap bmpDualcopter = new Bitmap(MultiWiiWinGUI.MWGUIControls.MWGUIControlsResources.dualcopter);
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            switch (CopterTypeToDraw)
            {
                ...
                case CopterType.Dualcopter:
                    bmpBi.MakeTransparent(Color.Yellow);
                    pe.Graphics.DrawRectangle(maskPen, 0, 0, bmpDualcopter.Width, bmpDualcopter.Height);
                    pe.Graphics.DrawImageUnscaled(bmpDualcopter, 0, 0, bmpDualcopter.Width, bmpDualcopter.Height);
                    for (i = 0; i < 2; i++)
                    {
                        int h = (int)((motorvals[i] - 900) * (float)(coord_dualcopter[i, 2] / 1200.0f));
                        pe.Graphics.FillRectangle(drawBrushGreen, coord_dualcopter[i, 0], coord_dualcopter[i, 1] - h, 10, h);
                        pe.Graphics.DrawString(String.Format("{0:0}", motorvals[i]), drawFont, drawBrush, coord_dualcopter[i, 0] + 22, coord_dualcopter[i, 1] - 10);
                    }
                    for (i = 0; i < 2; i++)
                    {
                        l = (int)((servovals[i] - 900) * (float)(coord_dualcopter[2 + i, 2] / 1200.0f));
                        pe.Graphics.FillRectangle(drawBrushGreen, coord_dualcopter[2 + i, 0], coord_dualcopter[2 + i, 1] - 10, l, 10);
                        pe.Graphics.DrawString(String.Format("{0:0}", servovals[i]), drawFont, drawBrush, coord_dualcopter[2 + i, 0], coord_dualcopter[2 + i, 1]);  //??? is servo 0 in new gui also?
                ...
                case CopterType.Hex6X:
                    ...
                    break;
                case CopterType.Hex6H:
                    bmpHex6H.MakeTransparent(Color.Yellow);
                    pe.Graphics.DrawRectangle(maskPen, 0, 0, bmpHex6H.Width, bmpHex6H.Height);
                    pe.Graphics.DrawImageUnscaled(bmpHex6H, 0, 0, bmpHex6H.Width, bmpHex6H.Height);
                    for (i = 0; i < 6; i++)
                    {
                        int h = (int)((motorvals[i] - 900) * (float)(coord_hex6h[i, 2] / 1200.0f));
                        pe.Graphics.FillRectangle(drawBrushGreen, coord_hex6h[i, 0], coord_hex6h[i, 1] - h, 8, h);
                        if (i > 3) { pe.Graphics.DrawString(String.Format("{0:0}", motorvals[i]), drawFont, drawBrush, coord_hex6h[i, 0] + 9, coord_hex6h[i, 1] -12); }
                        else { pe.Graphics.DrawString(String.Format("{0:0}", motorvals[i]), drawFont, drawBrush, coord_hex6h[i, 0] + 9, coord_hex6h[i, 1] - 12); }
                    }
                    break;
                ...
                case CopterType.Singlecopter:
                    bmpSinglecopter.MakeTransparent(Color.Yellow);
                    pe.Graphics.DrawRectangle(maskPen, 0, 0, bmpSinglecopter.Width, bmpSinglecopter.Height);
                    pe.Graphics.DrawImageUnscaled(bmpSinglecopter, 0, 0, bmpSinglecopter.Width, bmpSinglecopter.Height);

                    for (i = 0; i <= 4; i++)
                    {
                        hval = (int)((servovals[3 + i] - 900) * (float)(coord_singlecopter[i, 2] / 1200.0f));
                        if (i == 2) pe.Graphics.FillRectangle(drawBrushGreen, coord_singlecopter[i, 0], coord_singlecopter[i, 1] - 10, hval, 10);
                        else pe.Graphics.FillRectangle(drawBrushGreen, coord_singlecopter[i, 0], coord_singlecopter[i, 1] - hval, 10, hval);
                        pe.Graphics.DrawString(String.Format("{0:0}", servovals[3 + i]), drawFont, drawBrush, coord_singlecopter[i, 0], coord_singlecopter[i, 1] + 10);
